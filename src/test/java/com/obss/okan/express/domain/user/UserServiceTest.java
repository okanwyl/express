...
class UserServiceTest {
    ...
    @Test
    void when_create_user_execution_expect_password_encoded(@Mock CreateUserRequest request) {
    ...
    void when_login_expect_user_matches_password(@Mock Email email, @Mock User user) {
        given(userRepository.findByEmail(email)).willReturn(of(user));
    ...
    @Test
    void when_findByEmail_expect_repository_findByEmail(@Mock Email email) {
    ...
    //    @Test
//    void when_findByEmail_expect_repository_findFirstByProfileUserName(@Mock UserName userName) {
...
//    }
    ...
    @Test
    void when_updateUser_name_expect_user_changeName(@Mock User user, @Mock UserUpdateRequest request) {
        given(userRepository.findById(1L)).willReturn(of(user));
        given(request.getNameToUpdate()).willReturn(of("name"));

        userService.updateUser(1L, request);

        then(user).should(times(1)).changeName("name");
        verifyNoMoreInteractions(user);
    }

    @Test
    void when_updateUser_surname_expect_user_changeSurname(@Mock User user, @Mock UserUpdateRequest request) {
        given(userRepository.findById(1L)).willReturn(of(user));
        given(request.getSurnameToUpdate()).willReturn(of("surname"));

        userService.updateUser(1L, request);

        then(user).should(times(1)).changeSurname("surname");
        verifyNoMoreInteractions(user);
    }
